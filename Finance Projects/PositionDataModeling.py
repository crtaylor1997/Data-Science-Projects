...
def TransformPositionData(POS_PATH,data,early_date):
    ...
    Bpm['Portfolio'] = Bpm_header['The University of Texas/Texas A&M Investment Management Company'].loc[1]

    codes=pd.read_csv("redacted\\countries_codes_and_coordinates.csv")
    codes=codes[['Country','Alpha-3 code']]
    codes['Alpha-3 code']=codes['Alpha-3 code'].str.replace('"',"").str.strip()
    regions = pd.read_excel("redacted\\MSCI Regions.xlsx")
...
def MapBenchmarks(BMK_MAP_PATH,MGR_CODE_PATH,BMK_NAME_PATH):
    ...
    mgr_code.columns= ['BPM UTM Code','BPM Name']
    benchmark_mapping=benchmark_mapping.merge(mgr_code,how='left',on='BPM Name')
    bmk_name_map = pd.read_excel(BMK_NAME_PATH)
    bmk_name_map ={x:y for x,y in list(zip(bmk_name_map['Name'].tolist(),bmk_name_map['BPM Name'].tolist()))}
    benchmark_mapping=benchmark_mapping.replace(bmk_name_map).dropna(subset='Region BM')

    benchmark_mapping['BPM UTM Code'] = benchmark_mapping['BPM UTM Code'].str.replace('COMP_','')
...
def CalcBenchmarks(BMK_PATH):
    ...
    redacted_custom_BMK=pd.concat([MSCI_US_MID_VALUE,MSCI_US])
    ...
    redacted_custom_BMK = pd.concat([MSCI_US,MSCI_US_MIDCAP])
    ...
    redacted_custom_bmk = pd.concat([MSCI_AP,MSCI_ASIA_EX_JAPAN])

    #Create Dict of BPM Name of benchmarks to Transformed dataframes
    Benchmarks = {filename.split('.')[0].strip():TransformPositionData(BMK_PATH,filename,early_date) for filename in os.listdir(BMK_PATH)}
    Benchmarks['redacted Custom Benchmark'] = r_custom_BMK
    Benchmarks['redacted Custom Benchmark'] = re_custom_BMK 
    Benchmarks['redacted Custom BM'] = red_custom_bmk
    return Benchmarks
...
def CombinePositionData(benchmark_mapping,Benchmarks,Portfolio_Exp):
    ...
    #Loop Through each portfolio in the dataset, grab its assigned benchmarks from the dictionary that was built in the previous step and joins the two.
    for portfolio in Portfolio_Exp['Portfolio'].drop_duplicates().tolist():

        test_exp = Portfolio_Exp[Portfolio_Exp['Portfolio']==portfolio]
        test_benchmark_mapping=benchmark_mapping[benchmark_mapping['BPM UTM Code']==portfolio]
        ...
        test_exp=test_exp.merge(benchmark_mapping[['BPM UTM Code','BPM Name']], how='left',left_on='Portfolio',right_on='BPM UTM Code')
        Bpm_name = test_exp['BPM Name'].drop_duplicates().tolist()[0]
        ...
        agg=test_exp.groupby(['BPM Name','Date']).sum().reset_index()
        ...
        test_exp['BPM Name'] = test_exp['BPM Name'].fillna(Bpm_name)
        ...
        test_exp=test_exp.drop(columns=['BPM UTM Code','Mkt Value','Market Capitalization'])
    ...
    BPM_Exposure_data=pd.concat(portfolio)
    return(BPM_Exposure_data)

def topn(n,Bpm):
  
    b=Bpm[Bpm['Inst. Sub-type']!='Cash'][['Date','BPM Name','Asset Name','Portfolio (%)']]
    b=b.groupby(['Date','BPM Name','Asset Name']).sum().reset_index()
    b=b.sort_values(['Date','BPM Name','Portfolio (%)'],ascending=False).groupby(['Date','BPM Name']).head(n).reset_index(drop=True)

    b=b[['Date','BPM Name','Asset Name','Portfolio (%)']]

    managers = b['BPM Name'].drop_duplicates().tolist()
    ...
    for manager in managers:
        sb = b[b['BPM Name']==manager]
        sub_pos = sb.drop_duplicates(subset=['Asset Name'])[['Asset Name','BPM Name']]

        dates = b['Date'].drop_duplicates().tolist()
        for date in dates:
            sub_pos['Date']=date
            ch = sb[sb['Date']==date]
            ch=ch.merge(sub_pos,on=['Asset Name','BPM Name','Date'],how='outer')
            fs.append(ch)
    b=pd.concat(fs)

    for manager in managers:
        subset = b[b['BPM Name']==manager]
...
def PCTab(BPM_Exposure_data):
    ...
    b=b[['Date','BPM Name','Asset ID','Asset Name','Portfolio (%)','Holdings','BARRAID']]
    b['Holdings']=b['Holdings'].astype('float64')
    b=b.groupby(['Date','BPM Name','BARRAID','Asset Name']).sum().reset_index()
    ...
    for manager in managers:
        ...
        max_exp=sb[['Asset Name','BPM Name','BARRAID','Portfolio (%)']].groupby(['BARRAID']).max().reset_index()
        max_assets=max_exp[max_exp['Portfolio (%)']>=1.5]['BARRAID'].tolist()
        sb=sb[sb['BARRAID'].isin(max_assets)]

        sub_pos = sb.drop_duplicates(subset=['Asset Name'])[['Asset Name','BPM Name','BARRAID']]
        ...
        for date in dates:
            ...
            if ch.shape[0]==0:
                ...
                ch=ch.merge(sub_pos,on=['Asset Name','BPM Name','Date','BARRAID'],how='outer')
                fs.append(ch)
    b=pd.concat(fs)
    b=b.drop_duplicates()
    ...
    b.index=b['Date']
    b.index.name = None
    b['Prev_shares']=b.groupby(['BPM Name','Asset Name','BARRAID'])['Holdings'].shift(periods=1).fillna(0)
    b['Prev_Weight']=b.groupby(['BPM Name','Asset Name','BARRAID'])['Portfolio (%)'].shift(periods=1).fillna(0)
    ...
    # Good coding interview question
...
def FilterPC(PC):
    filtered_pc = PC[PC['Date']>='2023-03-31']
    dfs=[]
    for manager in filtered_pc['BPM Name'].drop_duplicates():
        x=filtered_pc[filtered_pc['BPM Name']==manager]
    ...
    filtered_pc['3 Month Lookback Exposure'] = filtered_pc.groupby(['BPM Name','Asset Name','BARRAID'])['Portfolio (%)'].shift(periods=3)
    filtered_pc['3 Month Lookback Holdings'] = filtered_pc.groupby(['BPM Name','Asset Name','BARRAID'])['Holdings'].shift(periods=3)
...
def CalculateExposureDeltas(col,BPM_Exposure_data):
    Bpm_Tearsheet = BPM_Exposure_data.groupby(['Date', 'BPM Name', col]).sum().reset_index()
    Bpm_Tearsheet['Prev_Year_Weight_Abs']=Bpm_Tearsheet.groupby(['BPM Name',col])['Portfolio (%)'].shift(periods=12).fillna(0)
    Bpm_Tearsheet['Prev_Year_Weight_Rel']=Bpm_Tearsheet.groupby(['BPM Name',col])['Manager Benchmark Exposure'].shift(periods=12).fillna(0)
    ...
    Bpm_Tearsheet['3M_Weight_Abs']=Bpm_Tearsheet.groupby(['BPM Name',col])['Portfolio (%)'].shift(periods=3).fillna(0)
    Bpm_Tearsheet['3M_Weight_Rel']=Bpm_Tearsheet.groupby(['BPM Name',col])['Manager Benchmark Exposure'].shift(periods=3).fillna(0)
    ...
    Bpm_Tearsheet['1M_Weight_Abs']=Bpm_Tearsheet.groupby(['BPM Name',col])['Portfolio (%)'].shift(periods=1).fillna(0)
    Bpm_Tearsheet['1M_Weight_Rel']=Bpm_Tearsheet.groupby(['BPM Name',col])['Manager Benchmark Exposure'].shift(periods=1).fillna(0)
    ...
    Bpm_Tearsheet['6M_Weight_Abs']=Bpm_Tearsheet.groupby(['BPM Name',col])['Portfolio (%)'].shift(periods=6).fillna(0)
    Bpm_Tearsheet['6M_Weight_Rel']=Bpm_Tearsheet.groupby(['BPM Name',col])['Manager Benchmark Exposure'].shift(periods=6).fillna(0)
    ...
    Sector_AVG=Bpm_Tearsheet.groupby(['BPM Name',col]).mean().reset_index().rename(columns={'Portfolio (%)':'LT Sector AVG ABS','Manager Benchmark Exposure':'LT Sector AVG REL'})[['BPM Name',col,	'LT Sector AVG ABS','LT Sector AVG REL'	]].drop_duplicates()

    Bpm_Tearsheet=Bpm_Tearsheet[Bpm_Tearsheet['Date']==Bpm_Tearsheet['Date'].max()]
    Bpm_Tearsheet=Bpm_Tearsheet[[ 'BPM Name','Date',col,'Portfolio (%)','Prev_Year_Weight_Abs', 'Prev_Year_Weight_Rel',  'YOY Change_Abs', 'YOY Change_Rel','6M Change_Abs','6M Change_Rel','1M Change_Abs','1M Change_Rel','3M Change_Abs','3M Change_Rel','3M_Weight_Abs','Manager Benchmark Exposure']]

    Bpm_Tearsheet=Bpm_Tearsheet.merge(Sector_AVG,how='left', on = ['BPM Name',col]).drop_duplicates()
...
def BarraFactorsTE(FACTOR_PATH):
    ...
    avgs = te[['Portfolio','Parent Node','Active Exp','% of TE Contribution']].groupby(['Portfolio','Parent Node']).mean().reset_index()
    avgs=avgs.rename(columns={'Active Exp':'Active Exp LT AVG','% of TE Contribution':'% of TE Contribution LT AVG'})

    te=te.merge(avgs, how='left', on=['Portfolio','Parent Node'])
...
if __name__ ==  '__main__':
    ...
    Modeled_Positions=CombinePositionData(benchmark_mapping,Benchmarks,Portfolio_Exp)

    Top_Ten=topn(10,Modeled_Positions)
    PC = PCTab(Modeled_Positions)
    Filtered_PC = FilterPC(PC)
    ExposureDeltas= CalculateExposureDeltas(col,Modeled_Positions)
